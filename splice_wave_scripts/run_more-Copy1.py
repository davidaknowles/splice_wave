...
model = tcn.TemporalConvNet([32] * 10, kernel_size=7, dropout = 0.2).to(device)

n_epoch = len(glob.glob("checkpoints_noncausal/*.pt"))
checkpoint = torch.load("checkpoints_noncausal/%i.pt" % (n_epoch-1))
checkpoint['model_state_dict']['out_proj.bias'] = checkpoint['model_state_dict']['causal_convs.10.bias']
checkpoint['model_state_dict']['out_proj.weight'] = checkpoint['model_state_dict']['causal_convs.10.weight']
del checkpoint['model_state_dict']['causal_convs.10.bias']
del checkpoint['model_state_dict']['causal_convs.10.weight']
...
def generate(model, one_hot, is_exon, context = 1, batch_size = 9):

    model.eval() # turn off dropout
    
    is_exon_gen = torch.ones(batch_size, 1, context, device = device)
    
    while is_exon_gen.shape[2] < is_exon.shape[2]:
        print(f"{context} / {is_exon.shape[2]}", end = "\r")
        one_hot_sub = one_hot[:,:,:context+2*model.receptive_field]
        output = model(is_exon_gen, one_hot_sub)
        
        new_is_exon = (torch.rand(batch_size).to(device) < output[:,0,-1].sigmoid()).float()
        is_exon_gen = torch.cat( (is_exon_gen, new_is_exon[:,None,None]), axis = 2)
        
        context += 1

    return is_exon_gen

is_exon_gen = generate(one_hot, is_exon)

is_exon_gen_np = is_exon_gen.cpu().numpy() 

for i in range(is_exon_gen_np.shape[0]):
    plt.plot(is_exon_gen_np[i,0,:])
