...
parser.add_argument("--cat", action="store_true", help="Categorical rather than binary rep of sequence")

args = parser.parse_args()
#args = parser.parse_args(["pred_x0", "--onehot", "--cat"])
...
unet = denoising_diffusion_1d.Unet1D(
    dim = 32,
    dim_mults = (1, 2, 4, 4, 4, 4, 8),
    channels = 9 if args.cat else (5 if args.onehot else 3),
    self_condition = args.self_cond
)

model = denoising_diffusion_1d.GaussianDiffusion1D(
    unet,
    likelihood = "cat" if args.cat else "binary",
...
if args.cat: assert( args.onehot )

checkpoint_path = Path((f"checkpoints_diffusion_binary_emb/{args.objective}_{args.self_cond}_{args.lr}")+("onehot" if args.onehot else "") + ("_cat" if args.cat else ""))
...
if n_epoch > 0: # restart from last checkpoint
