...
if False: 
    genome_urls = pd.read_csv(vertebrate_epigenomes / "genome_urls.txt", sep = " ")
    species = [ genome_urls.loc[i, "url"].split("/")[6] for i in range(genome_urls.shape[0]) ]
    species[0] = "ovis_aries"
    species[9] = "gallus_gallus"
    genome_urls["species"] = species
    genome_urls = genome_urls[ ["species", "genome", "url"] ]
    genome_urls.to_csv(vertebrate_epigenomes / "genome_urls.tsv", sep = "\t", index = False)
else: 
    genome_urls = pd.read_csv(vertebrate_epigenomes / "genome_urls.tsv", sep = "\t")


# clean up danio data
outdir = vertebrate_epigenomes / "danio_clean"
...
for f in files: 
    ...
    bed.to_csv(outdir / f.name, header=False, sep="\t", index=False)
...
# zcat GRCz11.fa.gz | awk '/^>/ {if($0 ~ /REF$/) {printit=1} else {printit=0}} {if(printit) print}' | gzip > GRCz11.fa.gz
import pickle
with open('genomes/GRCz11.pkl', 'wb') as f:
